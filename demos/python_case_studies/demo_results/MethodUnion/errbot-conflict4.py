...
import logging
import traceback
from backends.base import Backend,MUCRoom,Identifier,Room,Person
...
class ErrBot(Backend, StoreMixin):
    ...
    def __init__(self, bot_config):
        ...
        self.MSG_UNKNOWN_COMMAND = ('Unknown command: "%(command)s". Type "' +
            bot_config.BOT_PREFIX + 'help" for available commands.')
        if bot_config.BOT_ALT_PREFIX_CASEINSENSITIVE:
            self.bot_alt_prefixes = tuple(prefix.lower() for prefix in
                bot_config.BOT_ALT_PREFIXES)
        else:
    ...
    def send_templated(self, user_or_room, template_name, template_parameters,
        in_reply_to=None, groupchat_nick_reply=False):
    ...
    def split_and_send_message(self, mess):
        for part in split_string_after(mess.body, self.bot_config.
            MESSAGE_SIZE_LIMIT):
    ...
    def process_message(self, mess):
        ...
        if not hasattr(mess.frm, 'person'):
            raise Exception(
                'mess.frm not an Identifier as it misses the "person" property. Class of frm : %s'
                 % mess.frm.__class__)
        ...
        if mess.delayed:
            ...
            return False
        if (mess.is_direct and frm == self.bot_identifier or mess.is_group and 
            frm.nick == self.bot_config.CHATROOM_FN):
        ...
        prefixed = False
        only_check_re_command = False
        tomatch = text.lower(
            ) if self.bot_config.BOT_ALT_PREFIX_CASEINSENSITIVE else text
        if len(self.bot_config.BOT_ALT_PREFIXES) > 0 and tomatch.startswith(self
            .bot_alt_prefixes):
            prefixed = True
            longest = 0
        ...
        if not only_check_re_command:
            ...
            if not cmd:
                ...
                if command in self.commands:
                    ...
                    if len(text_split) > 1:
                        args = ' '.join(text_split[1:])
            if command == self.bot_config.BOT_PREFIX:
        ...
        if not cmd:
            if (prefixed or mess.is_direct and self.bot_config.
                BOT_PREFIX_OPTIONAL_ON_CHAT):
                commands = self.re_commands
            else:
                commands = {k: self.re_commands[k] for k in self.re_commands if
                    not self.re_commands[k]._err_command_prefix_required}
    ...
    def _process_command(self, mess, cmd, args, match):
        ...
        if (cmd, args) in user_cmd_history:
            user_cmd_history.remove((cmd, args))
        ...
        if f._err_command_historize:
            user_cmd_history.append((cmd, args))
        ...
        if self.bot_config.BOT_ASYNC:
            wr = WorkRequest(self._execute_and_send, [], {'cmd': cmd, 'args':
                args, 'match': match, 'mess': mess, 'template_name': f.
                _err_command_template})
    ...
    def process_template(template_name, template_parameters):
        if template_name:
            return tenv().get_template(template_name + '.md').render(**
                template_parameters)
    ...
    def _execute_and_send(self, cmd, args, match, mess, template_name=None):
        ...
        try:
            if inspect.isgeneratorfunction(commands[cmd]):
                replies = commands[cmd](mess, match) if match else commands[cmd](
                    mess, args)
                for reply in replies:
                    if reply:
                        self.send_simple_reply(mess, self.process_template(
                            template_name, reply), private)
            else:
            ...
            log.exception(
                'An error happened while processing a message ("%s"): %s"' % (
                mess.body, tb))
    ...
    def inject_commands_from(self, instance_to_inject):
        ...
        for name, value in inspect.getmembers(instance_to_inject, inspect.ismethod
            ...
            if getattr(value, '_err_command', False):
                commands = self.re_commands if getattr(value, '_err_re_command'
                    ) else self.commands
                ...
                if name in commands:
                    ...
                    self.warn_admins(
                        ...
                        (classname, name, type(f.__self__).__name__, f.__name__,
                        new_name))
                    name = new_name
                commands[name] = value
                if getattr(value, '_err_re_command'):
                    log.debug('Adding regex command : %s -> %s' % (name, value.
                        __name__))
    ...
    def remove_commands_from(self, instance_to_inject):
        for name, value in inspect.getmembers(instance_to_inject, inspect.ismethod
            ...
            if getattr(value, '_err_command', False):
                ...
                elif not getattr(value, '_err_re_command'
                    ) and name in self.commands:
    ...
    def signal_connect_to_all_plugins(self):
        for bot in self.plugin_manager.get_all_active_plugin_objects():
            if hasattr(bot, 'callback_connect'):
                try:
                    log.debug('Trigger callback_connect on %s' % bot.__class__.
                        __name__)
    ...
    def get_command_classes(self):
        return (get_class_that_defined_method(command) for command in self.
            all_commands.values())
