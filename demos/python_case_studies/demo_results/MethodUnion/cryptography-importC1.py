...
class TestCMAC(object):
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.
        cmac_algorithm_supported(TripleDES(fake_key)), skip_message=
        'Does not support CMAC.')
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.cipher_supported(
        ARC4(fake_key), None), skip_message='Does not support CMAC.')
