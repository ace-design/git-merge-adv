...
from errbot.backends.base import Message,Presence,Stream,MUCRoom,Identifier,ONLINE,Room
...
class BotPluginBase(StoreMixin):
    ...
    def start_poller(self, interval: float, method: Callable[..., None], args:
        Tuple=None, kwargs: Mapping=None):
    ...
    def stop_poller(self, method: Callable[..., None], args: Tuple=None, kwargs:
        Mapping=None):
        if not kwargs:
            kwargs = {}
        if not args:
            args = []
        log.debug('Stop polling of %s with args %s and kwargs %s' % (method,
            args, kwargs))
        self.current_pollers.remove((method, args, kwargs))
    
    def program_next_poll(self, interval: float, method: Callable[..., None],
        args: Tuple=None, kwargs: Mapping=None):
        t = Timer(interval=interval, function=self.poller, kwargs={'interval':
            interval, 'method': method, 'args': args, 'kwargs': kwargs})
        self.current_timers.append(t)
        t.setName('Poller thread for %s' % type(method.__self__).__name__)
        t.setDaemon(True)
        t.start()
    
    def poller(self, interval: float, method: Callable[..., None], args: Tuple=
        None, kwargs: Mapping=None):
...
class BotPlugin(BotPluginBase):
    ...
    def callback_mention(self, message: Message, mentioned_people: Sequence[
        Identifier]) ->None:
        """
...
>>>>>>> right_content.py
        ...
        if type(user_or_room) is str:
            raise ValueError(
                'user_or_send needs to be Identifier or MUCRoom, the old string behavior is not supported'
                )
        if message_type is not None:
            self.log.warn(
                'send message_type is DEPRECATED. Either pass a user identifier or a room to send.'
                )
    ...
    def send_templated(self, user: Identifier, template_name: str,
        template_parameters: Mapping, in_reply_to: Message=None, message_type:
        str=None, groupchat_nick_reply: bool=False) ->None:
        """
                Sends asynchronously a message to a room or a user.
            ...
            """
        return self._bot.send_templated(user, template_name,
            template_parameters, in_reply_to, message_type, groupchat_nick_reply)
    ...
    def send_stream_request(self, user: Identifier, fsource: IOBase, name: str=
        None, size: int=None, stream_type: str=None):
        """
