    ...
    def repos_install(self, mess, args):
        ...
        if errors:
            self.send(mess.frm, 'Some plugins are generating errors:\n' + '\n'.
                join(errors), message_type=mess.type)
        else:
            self.send(mess.frm, 
                'A new plugin repository has been installed correctly from %s. Refreshing the plugins commands...'
                 % args, message_type=mess.type)
...
>>>>>>> right_content.py
    ...
    def repos(self, mess, args):
        ...
        all_names = sorted(set([name for name in KNOWN_PUBLIC_REPOS] + [name for
            name in installed_repos]))
        repos = {'repos': []}
        for repo_name in all_names:
            installed = False
            public = False
            try:
                description = KNOWN_PUBLIC_REPOS[repo_name].get('documentation',
                    'Unavailable')
            except KeyError:
    ...
    def repos_update(self, mess, args):
        ...
        if not git_path:
            return (
                'git command not found: You need to have git installed on your system to be able to install git based plugins.'
                )
        directories = set()
        repos = {}
        ...
        for short_name, url in _installed.items():
            ...
            t_installed = {name: {'path': url, 'documentation': 'Unavilable',
                'python': None, 'avatar_url': None}}
            repos.update(t_installed)
        core_to_update = 'all' in args or 'core' in args
        if core_to_update:
            directories.add(path.dirname(__file__))
        if 'all' in args:
            directories.update([path.join(self._bot.plugin_dir, name) for name in
                repos])
        else:
            directories.update([path.join(self._bot.plugin_dir, name) for name in
                set(args).intersection(set(repos))])
        for d in directories:
            self.send(mess.frm, 'I am updating %s ...' % d, message_type=mess.type)
            p = subprocess.Popen([git_path, 'pull'], cwd=d, stdout=subprocess.
            ...
            if p.wait():
                self.send(mess.frm, 
                    'Update of %s failed...\n\n%s\n\n resuming...' % (d,
                    feedback), message_type=mess.type)
            else:
                self.send(mess.frm, 'Update of %s succeeded...\n\n%s\n\n' % (d,
                    feedback), message_type=mess.type)
                if not core_to_update:
                    for plugin in self._bot.getAllPlugins():
                        if plugin.path.startswith(d) and hasattr(plugin,
                            ...
                            self.send(mess.frm, '%s reloaded and reactivated' %
                                name)
        if core_to_update:
            self.send(mess.frm, 'You have updated the core, I need to restart.',
    ...
    def plugin_config(self, mess, args):
        ...
        if self._bot.is_plugin_blacklisted(plugin_name):
            return ('Load this plugin first with ' + self._bot.prefix + 
                'load %s' % plugin_name)
    ...
    {config}
    ```"""
                .format(prefix=self._bot.prefix, plugin_name=plugin_name,
    ...
    ```"""
        ...
        if type(real_config_obj) != type(template_obj):
            return (
                'It looks fishy, your config type is not the same as the template !'
                )
    ...
    def plugin_reload(self, mess, args):
        ...
        if name not in self._bot.get_all_plugin_names():
            yield "{} isn't a valid plugin name. The current plugins are:\n{}".format(
                name, self.formatted_plugin_list(active_only=False))
            return
