...
class TestMUCRoom(MUCRoom):
    ...
    def __init__(self, name, occupants=None, topic=None, bot=None):
        ...
        self._bot = bot
        self._name = name
        self._bot_mucid = TestMUCOccupant(self._bot.bot_config.BOT_IDENTITY[
            'username'], self._name)
    ...
    def join(self, username=None, password=None):
        if self.joined:
            logging.warning(
                "Attempted to join room '{!s}', but already in this room".
                format(self))
            return
        if not self.exists:
    ...
    def leave(self, reason=None):
        if not self.joined:
            logging.warning("Attempted to leave room '{!s}', but not in this room"
                .format(self))
            return
        room = self.find_croom()
    ...
    def destroy(self):
        if not self.exists:
            logging.warning("Cannot destroy room {!s}, it doesn't exist".format
                (self))
            return
...
class TestBackend(ErrBot):
    ...
    def __init__(self, config):
        ...
        self.sender = self.build_identifier(config.BOT_ADMINS[0])
        self.reset_rooms()
        self.md = text()
        ...
        try:
            while True:
                ...
                if stanza_type is STZ_MSG:
                    ...
                    mentioned = [self.build_identifier(word[1:]) for word in
                        entry.split() if word.startswith('@')]
                    if mentioned:
...
>>>>>>> right_content.py
    ...
    def start(self):
        ...
        self.bot_thread = Thread(target=self.bot.serve_forever, name=
            'TestBot main thread')
    ...
    def setup(self, extra_plugin_dir=None, loglevel=logging.DEBUG, extra_config
        =None):
        """
        ...
        config.__dict__.update(importlib.import_module('errbot.config-template'
            ).__dict__)
        config.BOT_DATA_DIR = tempdir
        config.BOT_LOG_FILE = tempdir + sep + 'log.txt'
...
class FullStackTest(unittest.TestCase, TestBot):
    def setUp(self, extra_plugin_dir=None, extra_test_file=None, loglevel=
        logging.DEBUG, extra_config=None):
        """
            :param extra_plugin_dir: Path to a directory from which additional
                plugins should be loaded.
...
def testbot(request):
    ...
    * `loglevel`
    ...
    logging.getLogger('Rocket.Errors').setLevel(logging.ERROR)
    logger.setLevel(logging.DEBUG)
    console_hdlr = logging.StreamHandler(sys.stdout)
    console_hdlr.setFormatter(logging.Formatter(
        '%(levelname)-8s %(name)-25s %(message)s'))
    logger.handlers = []
    logger.addHandler(console_hdlr)
    kwargs = {}
    for attr, default in (('extra_plugin_dir', None), ('loglevel', logging.
        DEBUG)):
