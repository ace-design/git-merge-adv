...
from apistar import core
DEFAULT_LOOKUP_CACHE_SIZE = 10000

class App(object):
    built_in_commands = cmd.new, cmd.run, cmd.schema, cmd.test
    def __init__(self, routes: routing.RoutesConfig=None, commands: List[
        Callable]=None, settings: Dict[str, Any]=None) ->None:
        from apistar.settings import Settings
        initial_types = [App, routing.Router]
...
def preload_state(state: Dict[str, Any], routes: routing.RoutesConfig) ->None:
    ...
    for component in components:
        ...
        pipeline = core.build_pipeline(function=builder, initial_types=[App])
