...
public class BlockBox extends Box implements InlinePaintable {
  ...
  @Override public String dump(LayoutContext c, String indent, int which) {
  ...
  @Override public Rectangle getPaintingClipEdge(CssContext cssCtx) {
  ...
  @Override public void paintInline(RenderingContext c) {
    if (!getStyle().isVisible(c, this)) {
      return;
    }
  ...
  @Override public void calcCanvasLocation() {
    if (isFloated()) {
    ...
    if (lineBox == null) {
      ...
      if (parent != null) {
        ...
        if (isStyled() && getStyle().isAbsFixedOrInlineBlockEquiv()) {
  ...
  @Override public void calcChildLocations() {
  ...
  @Override public void reset(LayoutContext c) {
    super.reset(c);
  ...
  private void sizeReplacedElement(LayoutContext c, ReplacedElement re) {
    ...
    if (cssWidth > 0 && cssHeight > 0) {
      if (useExact) {
        ...
        if (intrinsicWidth > cssWidth || intrinsicHeight > cssHeight) {
    ...
    if (!isDimensionsCalculated()) {
      ...
      if (!isAnonymous() || (isFromCaptionedTable() && isFloated())) {
        ...
        if (re != null) {
          ...
          if (cssWidth == -1 && pinnedContentWidth == -1 && style.isCanBeShrunkToFit()) {
...
<<<<<<< left.java
    if (isRoot()) {
      ...
      if (style.requiresLayer() && this.getLayer() == null) {
        pushedLayer = true;
        c.pushLayer(this);
      } else {
        if (style.requiresLayer()) {
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private void setCollapsedBottomMargin(LayoutContext c, RectPropertySet margin, MarginCollapseResult collapsedMargin) {
    ...
    if (!(next == null || next instanceof AnonymousBlockBox) && collapsedMargin.hasMargin()) {
      ...
      if ((int) margin.bottom() != collapsedMargin.getMargin()) {
      ...
      if (child.getStyle().isFloated() || child.getStyle().isInlineBlock() || child.getStyle().isInlineTable()) {
        ...
        if (whitespace == IdentValue.NOWRAP) {
          ...
          if (whitespace == IdentValue.PRE) {
            ...
            if (whitespace == IdentValue.PRE_WRAP || whitespace == IdentValue.PRE_LINE) {
              lineWidth += textIndent + iB.getFirstLineWidth();
              if (trimmableIB != null) {
                lineWidth -= trimmableIB.getTrailingSpaceWidth(c);
              }
  ...
  @Override protected void calcChildPaintingInfo(final CssContext c, final PaintingInfo result, final boolean useCache) {
    if (getPersistentBFC() != null) {
      (this).getPersistentBFC().getFloatManager().performFloatOperation(new FloatManager.FloatOperation() {
        @Override public void operate(Box floater) {
  ...
  @Override protected boolean isInlineBlock() {
    return isInline();
  }
  ...
  @Override public Box getDocumentParent() {
  ...
  public boolean checkPageContext(LayoutContext c) {
    if (!getStyle().isIdent(CSSName.PAGE, IdentValue.AUTO)) {
      ...
      if (c.getPageName() != null && isInDocumentFlow()) {
